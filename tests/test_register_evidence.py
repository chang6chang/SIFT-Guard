...
def fixture_file(tmp_path: Path) -> Path:
    """Create a 1 MB random file for registration tests."""
    fixture_path = tmp_path / "fixture.dat"
...
def case_dir(tmp_path: Path) -> Path:
    """Empty case directory inside the test's tmp_path."""
...
class TestRegisterEvidence:
    ...
    def test_second_registration_appends_chained_line(
        self, tmp_path: Path, case_dir: Path
    ):
        fix1 = tmp_path / "first.dat"
        fix2 = tmp_path / "second.dat"
    ...
    def test_nonexistent_path_raises_file_not_found(
        ...
        with pytest.raises(FileNotFoundError):
            register_evidence(
                str(tmp_path / "does-not-exist.dat"),
                case_dir=str(case_dir),
            )

    def test_nonexistent_path_does_not_create_audit_log(
        self, tmp_path: Path, case_dir: Path
    ):
        with pytest.raises(FileNotFoundError):
            register_evidence(
