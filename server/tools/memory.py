...
# Transport dispatch: `SIFT_VM_HOST` set means the operator wants
# the split-VM dev path (MCP server on one host, SIFT VM at
# host:2222 → guest:22). Unset means enterprise mode — the MCP
# server runs on the SIFT VM and `vol` is local. The two runners
# share parser surfaces; only `run_vol_plugin` / `get_vol_version`
# differ. Bound at import time so test patches on
# `server.tools.memory.run_vol_plugin` keep working unchanged.
if os.environ.get("SIFT_VM_HOST"):
    from server.runners.ssh_remote import get_vol_version, run_vol_plugin
else:
    from server.runners.local import get_vol_version, run_vol_plugin
