...
class Config:
    ...
    def report_formats(self) -> list[str]:
        fmt = (self.output.report_format or "both").lower()
        if fmt == "both":
        ...
        if fmt == "json":
            return ["json"]
        # Fall back to both rather than error — keeps sift-guard
        # forgiving on a typo'd config file.
        return ["markdown", "json"]


# Standard discovery order, first match wins.
_DEFAULT_DISCOVERY_PATHS: tuple[Path, ...] = (
...
def _config_from_doc(doc: dict[str, Any], source_path: Path | None) -> Config:
    ...
    return Config(
        ...
        output=OutputConfig(
            ...
            report_format=_coerce_str(out_doc.get("report_format")) or "both",
        ),
